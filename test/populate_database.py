...
def add_user_to_db(body: dict[str, str]) -> str:
    try:
        ...
        r: Res = post_to_url(f"{DOMAIN}:{PORT}/backend/users", body)
        return r.json()["id"]
    except Exception:
        r: Res = get_from_url(f"{DOMAIN}:{PORT}/backend/users")
        for i in r.json():
...
def get_user_access_token(body: dict[str, str]) -> str:
    try:
        r: Res = post_to_url(f"{DOMAIN}:{PORT}/backend/auth/login", body)
...
def create_game(
    ...
    post_to_url(f"{DOMAIN}:{PORT}/backend/games", body, users[winner]["token"])
