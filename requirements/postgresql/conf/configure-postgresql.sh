...
echo "[DB config] Configuring PostgreSQL..."

if [ ! -d "/run/mysqld" ]; then
	echo "[DB config] Granting PostgreSQL daemon run permissions..."
	mkdir -p /run/mysqld
...
then
	echo "[DB config] PostgreSQL already configured."
else
...
echo "[DB config] Allowing remote connections to PostgreSQL"
sed -i "s|skip-networking|# skip-networking|g" /etc/my.cnf.d/mariadb-server.cnf
sed -i "s|.*bind-address\s*=.*|bind-address=0.0.0.0|g" /etc/my.cnf.d/mariadb-server.cnf

echo "[DB config] Starting PostgreSQL daemon on port 5432."
