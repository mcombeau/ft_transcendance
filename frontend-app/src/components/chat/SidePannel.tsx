	...
	const channelInfo = (type: ChanType, channel?: ChatRoom, key?: number) => {
		...
		switch (type) {
			...
			default:
				...
				settingButton = (
					<button
						className={`border-2 border-darkblue text-sage dark:text-darksage m-2 rounded-md w-5 h-5 items-center ${
							settings && isCurrent
		...
		return (
			<div
				...
				className={`text-darkblue rounded-md p-2 m-2 flex ${
					isCurrent
						? "bg-sage dark:bg-darksage border-2 border-darkblue"
	...
	return (
		<>
			<form className="flex m-2 space-x-2" onSubmit={createChannel}>
				<input
					className="rounded-md flex-1 bg-sage dark:bg-darksage p-2 placeholder:text-darkblue placeholder:opacity-50 focus:outline-none"
					type="text"
				...
				/>
				<button className="bg-darkblue text-sage dark:text-darksage hover:bg-teal py-1 px-4 rounded-md hidden xl:block">
